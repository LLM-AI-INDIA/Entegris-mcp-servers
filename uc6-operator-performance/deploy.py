"""
Deploy MCP Server for ATHENA AI - Entegris KSP UC-6 Operator Performance Analysis.
Clones the UC-3 deploy structure (same VPC / subnet / SG / IAM role) but creates
a SEPARATE Lambda function and SEPARATE API Gateway so UC-3 and UC-6 are
independently deployable, monitored, and rolled back. Both servers can be
connected to a single LLM client side-by-side.

Entegris KSP UC-6 MCP Server — operator performance analytics.
...
# Naming — distinct from UC-1 and UC-3
FUNCTION_NAME = "entegris-ksp-uc6-operator-performance-mcp-server"
API_NAME = "entegris-ksp-uc6-operator-performance-mcp-api"

# Database Config — loaded from .env (gitignored). See .env.example.
REQUIRED_ENV_KEYS = [
    "DB_SERVER", "DB_PORT", "DB_INSTANCE", "DB_USERNAME", "DB_PASSWORD",
    "DB_NAME_DWH", "DB_NAME_OLTP", "DB_NAME_ODS", "ROW_LIMIT",
]
...
TAGS = [
    {"Key": "Project", "Value": "entegris-ksp-uc6-operator-performance"},
    {"Key": "Environment", "Value": "poc"},
    {
        "Key": "Description",
        "Value": "Entegris KSP UC-6 Operator Performance MCP Server",
    },
]

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# Try local sibling first, then bundle root
SHARED_INFRA_FILE_CANDIDATES = [
    os.path.join(SCRIPT_DIR, "..", "shared_infra.json"),
    os.path.join(SCRIPT_DIR, "..", "..", "shared_infra.json"),
]
...
def load_shared_infra(ec2):
    ...
    for sif in SHARED_INFRA_FILE_CANDIDATES:
        if os.path.exists(sif):
            with open(sif) as f:
                infra = json.load(f)
            print(f"  Loaded from {sif}")
...
def deploy_lambda(lambda_client, zip_path, role_arn, subnet_id, sg_id, db_config):
    ...
    if zip_size > 10:
        ...
        bucket_name = f"entegris-ksp-uc6-mcp-lambda-{REGION}"
        try:
    ...
    description = (
        "Entegris KSP UC-6 Operator Performance MCP - operator scorecards, "
        "learning curves, shift comparisons, downtime correlation. READ-ONLY. "
        "DBs: KSP DWH + OLTP. DB names + connection injected via env vars."
    )
...
def create_api_gateway(apigateway, lambda_client, function_arn):
    ...
    if existing_api:
        ...
        response = apigateway.create_api(
            ...
            ProtocolType="HTTP",
            Target=function_arn,
            Description="Entegris KSP UC-6 Operator Performance MCP API",
        )
...
def main():
    print("=" * 60)
    print("Deploying Entegris KSP UC-6 Operator Performance MCP Server")
    print("  13 tools + _discover_schema | DWH+OLTP | READ-ONLY")
    print("=" * 60)
    ...
    print(f"Tools:         13 + _discover_schema")
    print(f"Databases:     {db_config.get('DB_NAME_DWH', '(from env)')} + {db_config.get('DB_NAME_OLTP', '(from env)')} + {db_config.get('DB_NAME_ODS', '(from env)')}")
